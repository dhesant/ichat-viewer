    ...
    _.each(msgs, function (msg, index) {
      ...
      if (msg["Sender"] === null) {
        ...
        if (usrmsg.startsWith("/me")) {
          container = $(
            '<span class="system-message user-message">' +
              usrmsg.split("/me").join("<b>" + msg["Sender"]["ID"] + "</b>") +
              "</span>"
          );
        } else {
          var elem = $(
            '<span class="message" data-toggle="tooltip" data-placement="top" title="' +
              msg["Time"] +
              '">' +
              usrmsg +
              "</span>"
